...
const FellowshipLogsGatewayFightSchema = Schema.Struct({
...
export const FellowshipLogsGatewayFightResponseDataSchema = withRateLimitData({
  reportData: Schema.Struct({
    report: Schema.Struct({
      endTime: NonNegativeIntegerSchema,
      fights: Schema.Array(FellowshipLogsGatewayFightSchema),
    }).pipe(Schema.NullOr),
  }),
});
