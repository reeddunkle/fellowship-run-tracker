...
import { FellowshipLogsResponseCacheCompressionError } from "@frt/api/errors/fellowship-logs-response-cache-error.ts";
import {
  type FellowshipLogsCachedRequest,
  type FellowshipLogsResponseCacheShape,
} from "@frt/api/services/fellowship-logs/cache/fellowship-logs-response-cache-service.ts";
...
function compress(json: string) {
  return E.tryPromise({
      ...
      return new FellowshipLogsResponseCacheCompressionError({
        cause,
        operation: "Compress",
      });
    },
...
export const makeFellowshipLogsResponseCache = E.gen(function* () {
  const responseDAO = yield* FellowshipLogsResponseDAO;

  const read = E.fn("FellowshipLogsResponseCache.read")(function* <
  ...
  const write = E.fn("FellowshipLogsResponseCache.write")(function* <
    ResponseSchema extends Schema.Codec<unknown, unknown>,
  >(
    request: FellowshipLogsCachedRequest<ResponseSchema>,
  ...
  const cached: FellowshipLogsResponseCacheShape["cached"] = (
    request,
    fetch,
  ) => {
  ...
  return {
    cached,
  } satisfies FellowshipLogsResponseCacheShape;
});
