...
import { LiveSplitClientConnectionError } from "@frt/api/errors/live-split-client-error.ts";
import { AppSettings } from "@frt/api/services/app-settings/app-settings-service.ts";
import {
  type LiveSplitClientService,
  makeLiveSplitClient,
} from "@frt/api/services/live-split/core/live-split-client-service.ts";
import { makeNodeLiveSplitTransport } from "@frt/api/services/live-split/core/node-live-split-transport.ts";
...
export type LiveSplitConnectionManagerService = {
  ...
  readonly connect: () => E.Effect<void, LiveSplitClientConnectionError>;
...
const makeLiveSplitConnectionManager = E.gen(function* () {
  ...
  const connect: LiveSplitConnectionManagerService["connect"] = () => {
    const acquireClient = E.gen(function* () {
      ...
      E.mapError((cause) => {
        return new LiveSplitClientConnectionError({
          cause,
        });
      }),
    );
...
>()(
  "@frt/api/services/live-split/core/live-split-connection-manager-service/LiveSplitConnectionManager",
) {
