...
>()("fellowship-run-tracker/scripts/generate-layer-graph/EffectTsGoLsp") {}
...
const make = E.gen(function* () {
      ...
      const projectRoot = yield* fileSystem.realPath(".").pipe(
        E.mapError((cause) => {
          return new EffectTsGoLspWorkspaceError({
            cause,
            message: "Failed to resolve the project root.",
          });
        }),
      );
      ...
      const workspaceDirectory = yield* fileSystem
        ...
        .pipe(
          E.mapError((cause) => {
            return new EffectTsGoLspWorkspaceError({
              cause,
              message: "Failed to create the Effect TS-Go workspace.",
            });
          }),
        );
      ...
      yield* fileSystem.copy(sourceDirectory, workspaceSourceDirectory).pipe(
        E.mapError((cause) => {
          return new EffectTsGoLspWorkspaceError({
            cause,
            message:
              "Failed to copy the source tree into the Effect TS-Go workspace.",
          });
        }),
      );
      ...
      yield* fileSystem
        ...
        .pipe(
          E.mapError((cause) => {
            return new EffectTsGoLspWorkspaceError({
              cause,
              message: "Failed to write the Effect TS-Go workspace tsconfig.",
            });
          }),
        );
        ...
        .pipe(
          ...
          E.mapError((cause) => {
            return new EffectTsGoLspExecutableError({
              cause,
              message: `Failed to resolve the Effect TS-Go executable. ${dependencyMessage}`,
            });
          }),
        );
        ...
        .pipe(
          E.mapError((cause) => {
            return new EffectTsGoLspProcessError({
              cause,
              message: `Failed to launch the Effect TS-Go LSP. ${dependencyMessage}`,
            });
          }),
        );
        ...
        return encodeMessage(message).pipe(
          ...
          E.mapError((cause) => {
            return new EffectTsGoLspProtocolError({
              cause,
              message: "Failed to write a message to the Effect TS-Go LSP.",
            });
          }),
        );
      };
      ...
      const handleData = (
        ...
        return E.gen(function* () {
          ...
          yield* E.try({
              ...
              return new EffectTsGoLspProtocolError({
                cause,
                message: "Failed to parse a message from the Effect TS-Go LSP.",
              });
            },
            try: () => {
              while (true) {
      ...
      yield* child.stdout.pipe(
        ...
        E.catch((cause) => {
          const error = new EffectTsGoLspProcessError({
            cause,
            message: "Effect TS-Go LSP output stream failed.",
          });
        ...
        E.flatMap((exitCode) => {
          return failPendingRequests(
            new EffectTsGoLspProcessError({
              message: `Effect TS-Go LSP exited with code ${exitCode}.`,
            }),
          );
        }),
        E.catch((cause) => {
          return failPendingRequests(
            new EffectTsGoLspProcessError({
              cause,
              message: "Failed while waiting for the Effect TS-Go LSP process.",
            }),
          );
        }),
        E.forkScoped,
  ...
  const { notify, projectRoot, request, workspaceDirectory } = yield* setup();

  const getDocumentUri: EffectTsGoLspService["getDocumentUri"] = (
    sourceFile,
  ) => {
    return E.gen(function* () {
      const absoluteSourceFile = path.resolve(projectRoot, sourceFile);

      const relativeSourceFile = path.relative(projectRoot, absoluteSourceFile);
      ...
      if (
        ...
        return yield* new EffectTsGoLspSourceFileError({
          message: `Source file is outside the project root: ${sourceFile}`,
          sourceFile,
        });
      }

      return pathToFileURL(path.join(workspaceDirectory, relativeSourceFile))
        .href;
    });
  };
