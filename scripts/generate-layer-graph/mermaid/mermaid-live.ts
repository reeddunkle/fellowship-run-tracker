...
function getHoverMarkdown(response: JsonRpcResponse): string {
  if (
    ...
    throw new MermaidHoverResponseError({
      message: "Hover response did not contain contents.",
    });
  }
  ...
  if (
    ...
    throw new MermaidHoverResponseError({
      message: "Hover response did not contain Markdown contents.",
    });
  }
...
function getFullGraphUrl(markdown: string): string {
  ...
  if (url === undefined) {
    throw new MermaidGraphUrlError({
      message: "Could not find the full Mermaid graph URL.",
    });
  }
  ...
  try {
    ...
    if (!fragment.startsWith("#pako:")) {
      throw new MermaidGraphDecodeError({
        message: `Unsupported Mermaid URL: ${url}`,
        url,
      });
    }
    ...
    throw new MermaidGraphDecodeError({
      cause,
      message: `Failed to decode Mermaid graph URL: ${url}`,
      url,
    });
  }
}
